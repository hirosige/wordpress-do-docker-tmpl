 ...
 * 
 * This program was modified from MaxBlogPress Favicon plugin, version 2.0.0, 
...
define('cbnetpo_VERSION', '2.34');					// Current version of the Plugin
...
class cbnetPingOptimizer 
{
	...
	//Variable cbnetpo_request
	var $cbnetpo_request;	// Request Type Fixed  error
	 ...
	 */
	 // 1/21/2018 - rename function cbnetPingOptimizer to __construct
	public function __construct() {
		global $wp_version, $table_prefix, $post_title, $post_type;
		...
		//$this->cbnetpo_siteurl    = (strpos($this->cbnetpo_siteurl,'http://') === false) ? site_url() : $this->cbnetpo_siteurl;
		...
		register_activation_hook( $this->cbnetpo_path, array(&$this, 'cbnetpoActivate'));
		//add_action('deactivate_'.$this->cbnetpo_path, array(&$this, 'cbnetpoDeactivate'));
		register_deactivation_hook( $this->cbnetpo_path, array(&$this, 'cbnetpoDeactivate'));
...
*/
	...
	function cbnetpoAddMenu()	{
		add_options_page(cbnetpo_NAME, 'WordPress Ping Optimizer', 'manage_options', $this->cbnetpo_path, array(&$this, 'cbnetpoOptionsPg'));
	}
	...
	function cbnetpoFooter() {
		echo '<p style="text-align:center;margin-top:3em;"><strong>'.cbnetpo_NAME.' '.cbnetpo_VERSION.' by <a href="https://itspankajha.wordpress.com/portfolio/" target="_blank" >Author: Pakaj Jha</a></strong></p>';
	}
	...
	function cbnetpoOptionsPg() {
		global $wpdb, $msg, $errors;
		$msg ='';
		$errors='';
		if (isset($_REQUEST['cbnetpo'])) {
		    $this->cbnetpo_request = $_REQUEST['cbnetpo'];
        // below code added to WordPress plugin "WordPress ping Optimizer" vulnerable to cross-site scripting and cross-site request forgery
			if ($this->cbnetpo_request['uris'] != "") {
				$this->cbnetpo_request['uris'] = filter_var($this->cbnetpo_request['uris'], FILTER_SANITIZE_STRING);
				if ($this->cbnetpo_request['uris'] == "") {
					$errors .= 'Please enter a valid uris.<br/><br/>';
            }
			} else {
				$errors .= 'Please enter uris.<br/>';
			}

			if ($this->cbnetpo_request['limit_number'] != "") {
				$this->cbnetpo_request['limit_number'] = filter_var($this->cbnetpo_request['limit_number'], FILTER_SANITIZE_NUMBER_INT);
				if ($this->cbnetpo_request['limit_number'] == "") {
					$errors .= 'Please enter a valid limit number.<br/><br/>';
            }
			} else {
				$errors .= 'Please enter limit number.<br/>';
			}

			if ($this->cbnetpo_request['limit_time'] != "") {
				$this->cbnetpo_request['limit_time'] = filter_var($this->cbnetpo_request['limit_time'], FILTER_SANITIZE_NUMBER_INT);
				if ($this->cbnetpo_request['limit_time'] == "") {
					$errors .= 'Please enter a valid limit time.<br/><br/>';
            }
			} else {
				$errors .= 'Please enter limit time.<br/>';
			}

			// added FILTER_VALIDATE_BOOLEAN for ping and limit_ping 
			$this->cbnetpo_request['limit_ping'] = filter_var($this->cbnetpo_request['limit_ping'], FILTER_SANITIZE_NUMBER_INT);
		    $this->cbnetpo_request['ping'] = filter_var($this->cbnetpo_request['ping'], FILTER_SANITIZE_NUMBER_INT);
		}

			// added error condition for throwing error in case not SANITIZE
			if (!$errors) {
			...
			foreach ( (array) $ping_data as $ping_date => $_ping_data_arr ) {
				...
				foreach ( $_ping_data_arr as $key => $ping_data_arr ) {
					...
					if ( $ping_data_arr[2] == 2 || $ping_data_arr[2] == 3 || $ping_data_arr[2] == 4 ) {	
						...
						// handle null before count
						$cntCount=0;
						if (is_null($_ping_data_arr))
							$cntCount=0;
						else
							$cntCount=count($_ping_data_arr);
						if ( $cntCount == $cnt ) $msg .= '<br></div>';
					} else {
